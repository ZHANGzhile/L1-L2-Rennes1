...
//   Optionnel : vous pouvez aussi joindre un document PDF donnant des explications supplementaires (si vous utilisez OpenOffice/LibreOffice/Word, exportez le document en PDF), avec eventuellement des captures d'ecran montrant des étapes affichees dans la console
//
...
//
// - Enfin, n'hesitez pas à contacter l'equipe pedagogique, en posant une question sur le forum du Moodle, si quelque chose n'est pas clair.
//
...
// utilises en questions 4.d] pour la fonction jeu()
    ...
    public class projet_demineur_MatheoGUEGAN_ZhileZHANG_G4B {
     
        // ne, utile pour la question 1.b]
        ...
        static int nombreDePartie = 0;
        static int nombreDePartieGagne = 0;
        ...
        // Question 4.d]
        public static void jeu(boolean ia) { //le boolean ia va permettrede definir si c'est un joueur ou une ia qui lance la partie
        	boolean perdu = false;
        	boolean gagne = false;
        	...
        	while(!perdu && !gagne) {
	        	...
	        	while(inputError) {
		        	...
		        	if(input.indexOf("ia") >= 0) {// si la chaine input contient "ia"
		        		ia=true;   // vous aurez une IA pour terminer le jeu a votre place
		        		inputError=false;
		        		aide=true;
		        	}
		        	else if(input.indexOf("aide") >= 0) {// si la chaine input contient "aide"
		        		aide(false);  //vous aurez de l'aide
		        		aide=true;
		        		inputError=false;
		        	}
        ...
        public static int[] aide(boolean ia) { //le boolean ia va permetre de savoir si lma fonction ets utilisé par l'ia ou pas le joueur
        	...
        	for(int i=0;i<Tadj.length;i++) { //on parcour la grille en largeur
        	    for(int j=0;j<Tadj[0].length;j++) { //on parcour la grille en hauteur
        		    ...
        		    if(hideCase==bombN && bombN>0){ //si il y a autant de mines autour de la case que de case chaché autour alors on peut placer des drapeau sur les case cachés autour
        		    ...
        		    if(bombN==0 && Tadj[i][j]>0 && hideCase>0) {//si autour de la case donne sauf la case qui a le drapeau c'est sûr qu'il n'y a pas d'autre mine
        ...
        public static String intelligenceArtificielle() {
            ...
            }else if(instruction[0]==0) {//si la fonction "aide" indique une case qu'il faudra mettre un drapeau
        ...
        public static void statistiquesVictoiresIA() {
        	...
        	String partie301699= nombreDePartieGagne+""; // on sauvegarde le nombre de partie pour la grille de 30x16 avec 99 mines
        	nombreDePartieGagne=0;
