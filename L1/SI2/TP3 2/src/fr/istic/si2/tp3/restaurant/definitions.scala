...
object Definitions {
  ...
  def prixFormule(f: Formule): Int = {
    ...
    case Complete(e, p, d) =>  prixEntree(e) + prixPlat(p) + prixDessert(d) - 2
  ...
  def changerDessert(f: Formule, nd: Dessert): Formule = {
    ...
    f match {
      ...
      case PlatDessert(p,_) => PlatDessert(p,nd)
      case Complete(e,p,_) => Complete(e,p,nd)
  ...
  def changerOuAnnulerDessert(f: Formule, ndo: Option[Dessert]): Formule = {
    ...
    ndo match {
      ...
      case Some(d) => changerDessert(f,d)
      
      case None => f match {
      
                      case PetiteFaim(p) => PetiteFaim(p)
                      case EntreePlat(e,p) => EntreePlat(e,p)
                      case PlatDessert(p,_) => PetiteFaim(p)
                      case Complete(e,p,_) => EntreePlat(e,p)
